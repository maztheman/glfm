...
#define glfm__wasJavaExceptionThrown() ((*jni)->ExceptionCheck(jni) ? ((*jni)->ExceptionClear(jni), true) : false)
...
#define glfm__callJavaMethod(jni, object, methodName, methodSig, returnType) \
    (*jni)->Call##returnType##Method(jni, object, glfm__getJavaMethodID(jni, object, methodName, methodSig))

#define glfm__callJavaMethodWithArgs(jni, object, methodName, methodSig, returnType, ...) \
    (*jni)->Call##returnType##Method(                                                     \
      jni, object, glfm__getJavaMethodID(jni, object, methodName, methodSig), __VA_ARGS__)

#define glfm__getJavaField(jni, object, fieldName, fieldSig, fieldType) \
    (*jni)->Get##fieldType##Field(jni, object, glfm__getJavaFieldID(jni, object, fieldName, fieldSig))

#define glfm__getJavaStaticField(jni, class, fieldName, fieldSig, fieldType) \
    (*jni)->GetStatic##fieldType##Field(jni, class, glfm__getJavaStaticFieldID(jni, class, fieldName, fieldSig))
...
static void glfm__setOrientation(struct android_app *app)
{
    ...
    bool portraitRequested = (((uint8_t)orientations & (uint8_t)GLFMInterfaceOrientationPortrait)
...
static void glfm__setFullScreen(struct android_app *app, GLFMUserInterfaceChrome uiChrome)
{
    ...
    else if (SDK_INT >= 14 && SDK_INT < 19)
    {
        ...
        {
            glfm__callJavaMethodWithArgs(
              jni, decorView, "setSystemUiVisibility", "(I)V", Void, (jint)View_SYSTEM_UI_FLAG_FULLSCREEN);
        }
        else
        {
            glfm__callJavaMethodWithArgs(jni,
              decorView,
    ...
    else if (SDK_INT >= 19)
    {
        ...
        else
        {
            glfm__callJavaMethodWithArgs(jni,
              ...
              "(I)V",
              Void,
              (jint)(View_SYSTEM_UI_FLAG_HIDE_NAVIGATION | View_SYSTEM_UI_FLAG_FULLSCREEN
                     | View_SYSTEM_UI_FLAG_LAYOUT_STABLE | View_SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION
                     | View_SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View_SYSTEM_UI_FLAG_IMMERSIVE_STICKY));
        }
    }
...
static bool glfm__handleBackButton(struct android_app *app)
{
    ...
    jboolean handled =
      glfm__callJavaMethodWithArgs(jni, app->activity->clazz, "moveTaskToBack", "(Z)Z", Boolean, false);
...
static bool glfm__setKeyboardVisible(GLFMPlatformData *platformData, bool visible)
{
    ...
    jstring imString =
      glfm__getJavaStaticField(jni, contextClass, "INPUT_METHOD_SERVICE", "Ljava/lang/String;", Object);
    if (!imString || glfm__wasJavaExceptionThrown())
    {
        return false;
    }
    jobject ime = glfm__callJavaMethodWithArgs(jni,
      platformData->app->activity->clazz,
      "getSystemService",
    ...
    {
        glfm__callJavaMethodWithArgs(
          jni, ime, "showSoftInput", "(Landroid/view/View;I)Z", Boolean, decorView, InputMethodManager_SHOW_FORCED);
    }
    else
    {
        ...
        glfm__callJavaMethodWithArgs(
          jni, ime, "hideSoftInputFromWindow", "(Landroid/os/IBinder;I)Z", Boolean, windowToken, 0);
...
static void glfm__resetContentRect(GLFMPlatformData *platformData)
{
    ...
    jfieldID field = glfm__getJavaFieldID(jni, platformData->app->activity->clazz, "mLastContentWidth", "I");
...
static ARect glfm__getWindowVisibleDisplayFrame(GLFMPlatformData *platformData, ARect defaultRect)
{
    ...
    glfm__callJavaMethodWithArgs(
      jni, decorView, "getWindowVisibleDisplayFrame", "(Landroid/graphics/Rect;)V", Void, javaRect);
    if (glfm__wasJavaExceptionThrown())
    {
        return defaultRect;
    }
...
static uint32_t glfm__getUnicodeChar(GLFMPlatformData *platformData, AInputEvent *event)
{
    ...
    jobject eventObject =
      (*jni)->NewObject(jni, keyEventClass, eventConstructor, (jint)AKEY_EVENT_ACTION_DOWN, keyCode);
...
static bool glfm__eglContextInit(GLFMPlatformData *platformData)
{
    ...
    if (platformData->eglContext == EGL_NO_CONTEXT)
    {
        ...
        if (platformData->display->preferredAPI >= GLFMRenderingAPIOpenGLES32)
        {
            ...
            const EGLint contextAttribs[] = {
                EGL_CONTEXT_MAJOR_VERSION_KHR, majorVersion, EGL_CONTEXT_MINOR_VERSION_KHR, minorVersion, EGL_NONE
            };
            platformData->eglContext =
              eglCreateContext(platformData->eglDisplay, platformData->eglConfig, EGL_NO_CONTEXT, contextAttribs);
        ...
        if (!created && platformData->display->preferredAPI >= GLFMRenderingAPIOpenGLES3)
        {
            majorVersion = 3;
            minorVersion = 0;
            const EGLint contextAttribs[] = { EGL_CONTEXT_CLIENT_VERSION, majorVersion, EGL_NONE, EGL_NONE };
            platformData->eglContext =
              eglCreateContext(platformData->eglDisplay, platformData->eglConfig, EGL_NO_CONTEXT, contextAttribs);
            created = platformData->eglContext != EGL_NO_CONTEXT;
        }
        ...
        {
            eglQueryContext(
              platformData->eglDisplay, platformData->eglContext, EGL_CONTEXT_MAJOR_VERSION_KHR, &majorVersion);
            if (majorVersion >= 3)
            {
                // This call fails on many (all?) devices.
                // When it fails, `minorVersion` is left unchanged.
                eglQueryContext(
                  platformData->eglDisplay, platformData->eglContext, EGL_CONTEXT_MINOR_VERSION_KHR, &minorVersion);
            }
    ...
    if (!eglMakeCurrent(
          platformData->eglDisplay, platformData->eglSurface, platformData->eglSurface, platformData->eglContext))
    {
    ...
    else
    {
        ...
        if (created && platformData->display)
        {
            ...
            {
                platformData->display->surfaceCreatedFunc(
                  platformData->display, platformData->width, platformData->height);
            }
        }
        return true;
    }
...
static void glfm__eglSurfaceInit(GLFMPlatformData *platformData)
{
    ...
    {
        platformData->eglSurface =
          eglCreateWindowSurface(platformData->eglDisplay, platformData->eglConfig, platformData->app->window, NULL);
        ...
        case GLFMSwapBehaviorBufferPreserved:
            eglSurfaceAttrib(
              platformData->eglDisplay, platformData->eglSurface, EGL_SWAP_BEHAVIOR, EGL_BUFFER_PRESERVED);
            break;
        case GLFMSwapBehaviorBufferDestroyed:
            eglSurfaceAttrib(
              platformData->eglDisplay, platformData->eglSurface, EGL_SWAP_BEHAVIOR, EGL_BUFFER_DESTROYED);
        }
    }
}
...
static bool glfm__eglInit(GLFMPlatformData *platformData)
{
    ...
    eglQuerySurface(platformData->eglDisplay, platformData->eglSurface, EGL_WIDTH, &platformData->width);
    eglQuerySurface(platformData->eglDisplay, platformData->eglSurface, EGL_HEIGHT, &platformData->height);
    eglGetConfigAttrib(platformData->eglDisplay, platformData->eglConfig, EGL_NATIVE_VISUAL_ID, &format);
            ...
            || !ARectsEqual(platformData->keyboardFrame, keyboardFrame))
        {
            ...
            if (platformData->display->keyboardVisibilityChangedFunc)
            {
                ...
                platformData->display->keyboardVisibilityChangedFunc(
                  platformData->display, keyboardVisible, x, y, w, h);
            }
        }
    }
}
    ...
    else if (eventType == AINPUT_EVENT_TYPE_MOTION)
    {
        if (platformData->display && platformData->display->touchFunc)
        {
            ...
            if (validAction)
            {
                if (phase == GLFMTouchPhaseMoved)
                {
                    ...
                    for (i = 0; i < count; i++)
                    {
                        ...
                        if (touchNumber >= 0 && touchNumber < maxTouches)
                        {
                            ...
                            platformData->display->touchFunc(platformData->display, touchNumber, phase, x, y);
                ...
                {
                    const size_t index = (size_t)(((uint32_t)action & (uint32_t)AMOTION_EVENT_ACTION_POINTER_INDEX_MASK)
...
{
    if (hint == GLFW_APP_DATA)
    {
        s_App = (struct android_app *)value;
    }
}
...
#pragma clang diagnostic pop
    ...
    struct android_app *app = platformData->app = s_App;
    ...
    if (!windowAttributesSet)
    {
        ...
        if (SDK_INT >= 28)
        {
            ...
            jobject window =
              glfm__callJavaMethod(jni, app->activity->clazz, "getWindow", "()Landroid/view/Window;", Object);
            jobject attributes =
              glfm__callJavaMethod(jni, window, "getAttributes", "()Landroid/view/WindowManager$LayoutParams;", Object);
            ...
            (*jni)->SetIntField(jni, attributes, layoutInDisplayCutoutMode, LAYOUT_IN_DISPLAY_CUTOUT_MODE_SHORT_EDGES);
...
int glfmAppShouldClose()
{
    ...
    return app == NULL ? GLFM_TRUE : GLFM_FALSE;
}
...
void glfmDisplayHint(int hint, int value)
{
    ...
    if (hint == GLFM_CONTEXT_RENDER_API)
    {
        platformData->renderingAPI = value;
    }
}
...
GLFMDisplay *glfmCreateDisplay()
{
    ...
    if (platformData->display == NULL)
    {
        ...
        // use hints to figure out what api to use
        glfmSetDisplayConfig(platformData->display,
          platformData->renderingAPI,
    ...
    ANativeActivity_setWindowFormat(s_App->activity, windowFormat);
    ANativeActivity_setWindowFlags(s_App->activity, fullscreen ? AWINDOW_FLAG_FULLSCREEN : 0, AWINDOW_FLAG_FULLSCREEN);
    glfm__setFullScreen(app, platformData->display->uiChrome);

    return platformData->display;
}
...
void glfmPollEvents()
{
    ...
    while ((eventIdentifier = ALooper_pollAll(platformData->animating ? 0 : -1, NULL, &events, (void **)&source)) >= 0)
    {
        if (source)
        {
...
static float glfm__getRefreshRate(GLFMDisplay *display)
{
    ...
    glfm__clearJavaException() jobject window =
      glfm__callJavaMethod(jni, activity, "getWindow", "()Landroid/view/Window;", Object);
    if (!window || glfm__wasJavaExceptionThrown())
    {
        return 60;
    }
    jobject windowManager =
      glfm__callJavaMethod(jni, window, "getWindowManager", "()Landroid/view/WindowManager;", Object);
    (*jni)->DeleteLocalRef(jni, window);
    if (!windowManager || glfm__wasJavaExceptionThrown())
    {
        return 60;
    }
    jobject windowDisplay =
      glfm__callJavaMethod(jni, windowManager, "getDefaultDisplay", "()Landroid/view/Display;", Object);
...
static bool glfm__getSafeInsets(GLFMDisplay *display, double *top, double *right, double *bottom, double *left)
{
    ...
    jobject insets =
      glfm__callJavaMethod(jni, decorView, "getRootWindowInsets", "()Landroid/view/WindowInsets;", Object);
    (*jni)->DeleteLocalRef(jni, decorView);
    if (!insets)
    {
        return false;
...
void glfmGetDisplayChromeInsets(GLFMDisplay *display, double *top, double *right, double *bottom, double *left)
{
...
static void glfm__setAllRequestedSensorsEnabled(GLFMDisplay *display, bool enabledGlobally)
{
    ...
    for (int i = 0; i < GLFM_NUM_SENSORS; i++)
    {
        ...
        {
            ASensorManager *sensorManager = ASensorManager_getInstance();
            platformData->sensorEventQueue = ASensorManager_createEventQueue(
              sensorManager, ALooper_forThread(), LOOPER_ID_SENSOR_EVENT_QUEUE, NULL, NULL);
            if (!platformData->sensorEventQueue)
            {
                continue;
            }
...
bool glfmIsHapticFeedbackSupported(GLFMDisplay *display)
{
    ...
    */
    ...
    jstring vibratorServiceString =
      glfm__getJavaStaticField(jni, contextClass, "VIBRATOR_SERVICE", "Ljava/lang/String;", Object);
    if (!vibratorServiceString || glfm__wasJavaExceptionThrown())
    {
        (*jni)->DeleteLocalRef(jni, contextClass);
        return false;
    }
    jobject vibratorService = glfm__callJavaMethodWithArgs(jni,
      platformData->app->activity->clazz,
      "getSystemService",
      "(Ljava/lang/String;)Ljava/lang/Object;",
      Object,
      vibratorServiceString);
...
void glfmPerformHapticFeedback(GLFMDisplay *display, GLFMHapticFeedbackStyle style)
{
    ...
    jint feedbackFlags =
      HapticFeedbackConstants_FLAG_IGNORE_VIEW_SETTING | HapticFeedbackConstants_FLAG_IGNORE_GLOBAL_SETTING;
    switch (style)
    {
    ...
    bool performed = glfm__callJavaMethodWithArgs(
      jni, decorView, "performHapticFeedback", "(II)Z", Boolean, feedbackConstant, feedbackFlags);
    if (!performed)
    {
        // Some devices (Samsung S8) don't support all constants
        glfm__callJavaMethodWithArgs(
          jni, decorView, "performHapticFeedback", "(II)Z", Boolean, defaultFeedbackConstant, feedbackFlags);
    }
    (*jni)->DeleteLocalRef(jni, decorView);
}
